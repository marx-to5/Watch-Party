...
export default function RoomPage() {
  ...
  useEffect(() => {
    ...
    const unsub = onValue(vsRef, (snap) => {
      ...
      syncStateRef.current = {
        ...
        lastUpdated: vs.lastUpdated ?? getSyncedTime()
      };
      ...
      if (playerRef.current && vs.currentTime !== undefined) {
        ...
        const elapsed = vs.lastUpdated
          ? Math.max(0, (getSyncedTime() - vs.lastUpdated) / 1000)
          : 0;
  ...
  const enforceGuestSync = useCallback(() => {
    ...
    const elapsed = lastUpdated ? Math.max(0, (Date.now() + serverTimeOffsetRef.current - lastUpdated) / 1000) : 0;
  ...
  const handleReady = useCallback(async () => {
    ...
    try {
      ...
      if (vs && playerRef.current) {
        isSyncingRef.current = true;
        const elapsed = vs.lastUpdated
          ? Math.max(0, (Date.now() + serverTimeOffsetRef.current - vs.lastUpdated) / 1000)
          : 0;
        const target = Math.max(0, vs.isPlaying ? vs.currentTime + elapsed : vs.currentTime);
        if (playerType === "youtube" && playerRef.current.seekTo) {
