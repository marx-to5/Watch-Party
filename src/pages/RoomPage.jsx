...
export default function RoomPage() {
  ...
  const reactPlayerUrl = useMemo(() => toReactPlayerUrl(videoUrl), [videoUrl]);
  ...
  // ── Firebase: Presence count + auto-delete when room empties ──────────────
  useEffect(() => {
    ...
    const unsub = onValue(presRef, (snap) => {
      ...
      // Auto-delete: if all participants dropped (e.g., others lost connection)
      // while we're still actively watching, we're the last one — delete & leave.
      // Guard: only run after the user has entered the room (usernameSet)
      // and the count just hit 0 (our own presence was removed elsewhere).
  ...
  const registerPresence = useCallback(async (name) => {
    ...
    await set(pRef, { name, joinedAt: Date.now() });
      ...
      <header className="room-header">
        ...
        <div className="room-header-center">
          <div className="room-id-badge">
            {isAdmin && <LockOutlined style={{ color: "#facc15", fontSize: 11 }} />}
            <span className="room-id-label">{roomName}</span>
            <span className="room-id-sep">·</span>
        ...
        <div className="room-header-right">
          ...
          <div className={`participant-counter ${isFull ? "counter-full" : ""}`}>
        ...
        <div className="chat-panel">
          ...
          <div className="chat-messages">
            {!chatEnabled ? (
              <div className="chat-empty">
                <MessageOutlined style={{ fontSize: 24, color: "#1e2535", marginBottom: 8 }} />
                <Text style={{ color: "#334155", fontSize: 13 }}>Chat has been disabled for this room</Text>
              </div>
            ) : messages.length === 0 ? (
              <div className="chat-empty">
                <Text style={{ color: "#334155", fontSize: 13 }}>No messages yet. Say hello! 👋</Text>
              </div>
            ) : (
              messages.map((msg, i) => {
                const isOwn = msg.user === username;
                return (
                  <div key={msg.id || i} className={`chat-message ${isOwn ? "chat-message-own" : ""} slide-in-right`}>
                    {!isOwn && <div className="chat-avatar">{msg.user?.[0]?.toUpperCase() || "?"}</div>}
                    <div className="chat-bubble-group">
                      {!isOwn && <span className="chat-username">{msg.user}</span>}
                      <div className={`chat-bubble ${isOwn ? "chat-bubble-own" : ""}`}>
                        <span className="chat-text">{msg.text}</span>
                        <span className="chat-time">{formatTime(msg.timestamp)}</span>
                      </div>
                    </div>
                    {isOwn && <div className="chat-avatar chat-avatar-own">{username?.[0]?.toUpperCase() || "?"}</div>}
                  </div>
                );
              })
            )}
          ...
          <div className="chat-input-bar">
            <Input
              ...
              onPressEnter={chatEnabled ? handleSendMessage : undefined}
              disabled={!chatEnabled}
              style={{ flex: 1, background: "#060612", borderColor: "#1f1f2e" }}
            />
            <Button
              id="send-msg-btn"
              type="primary"
