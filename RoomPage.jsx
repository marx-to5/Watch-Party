...
export default function RoomPage() {
  ...
  const getReactionPopoverContent = (msgId) => (
    <div className="reaction-picker-container">
      {REACTION_EMOJIS.map(em => (
        <span 
          key={em} 
          onClick={() => handleReaction(msgId, em)}
      ...
      <div className="room-layout">
        ...
        <div className="chat-panel">
                  ...
                  <div key={msg.id || i} className={`chat-message ${isOwn ? "chat-message-own" : ""} ${isConsecutive ? "chat-message-consecutive" : ""} slide-in-right`} style={{ marginTop: isConsecutive ? -6 : 0 }}>
                    ...
                    <div className="chat-bubble-group">
                      ...
                      <div className={`chat-bubble-row ${isOwn ? "chat-bubble-row-own" : ""}`}>
                        ...
                        {(chatEnabled && !isMuted && !isOwn) && (
                          <div className="chat-msg-react-wrap">
                            <Popover 
                              content={getReactionPopoverContent(msg.id)} 
                              trigger="click"
                              placement="top"
                              overlayInnerStyle={{ background: 'rgba(15, 23, 42, 0.85)', backdropFilter: 'blur(8px)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '20px' }}
                            >
                              <SmileOutlined className="chat-msg-react-icon" />
                            </Popover>
                          </div>
                        )}
                      </div>
          ...
          <div className="chat-input-bar">
            <Input
              id="chat-input"
              placeholder={!chatEnabled ? "Chat is disabled" : isMuted ? "You have been muted" : "Type a message…"}
              value={chatInput}
              onChange={handleChatInputChange}
              onPressEnter={(chatEnabled && !isMuted) ? handleSendMessage : undefined}
              disabled={!chatEnabled || isMuted}
              style={{ flex: 1, background: "#060612", borderColor: isMuted ? "rgba(239,68,68,0.3)" : "#1f1f2e" }}
            />
            <Button
              id="send-msg-btn"
              type="primary"
